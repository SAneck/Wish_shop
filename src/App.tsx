	...
	return (
		<div className={classes.container}>
			...
			<div className={classes.catalog}>
				...
				<h1>Каталог</h1>
				<input onChange={e => setSearchInputValue(e.target.value)} value={searchInputValue} type='text' placeholder='Искать'></input>
				<button>Поиск</button>
				<div>
					<form onSubmit={addCard}>
						<input placeholder='Image' 
							onChange={e => setAddInputValue(prev => ({...prev, image: e.target.value}))}
							value={addInputValue.image}
						/>
						<input placeholder='Title' 
							onChange={e => setAddInputValue(prev => ({...prev, title: e.target.value}))}
							value={addInputValue.title}
						/>
						<input placeholder='Price' 
							onChange={e => setAddInputValue(prev => ({...prev, price: e.target.value}))}
